...
def create_data_set(look_back, mean_label, year):

    stocks = Stocks(year=year, cod=model['code'], period=2)
    dataset = stocks.selected_fields([CLOSING])
    stocks = Stocks(year=year+1, cod=model['code'], period=2)
...
for year in range(2014, 2018):
    index_names.append(str(year) + '-' + str(year+1) + '-0')
    index_names.append(str(year) + '-' + str(year+1) + '-1')

df = pandas.DataFrame(columns=['PETR3', 'ABEV3', 'VALE3'], index=index_names)
for value in label_choice:
    for model in models:
        for year in tqdm(range(2014, 2018)):
            train_x, train_y, test_x, test_y, look_back = create_data_set(model['window_size'], value, year)
            ...
            index = str(year) + '-' + str(year+1) + '-' + str(value)

            df.loc[index, [model['code']]] = acc

            print(df)
            K.clear_session()

df.to_csv('../results/text_next_year_3_months' + ".csv")
