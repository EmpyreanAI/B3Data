...
for period in periods:
    ...
    for alpha in alphas:
    	...
    	for beta in betas:
	        ...
	        model = ExponentialSmoothing(list(data), trend='add')
	        ...
	        register = {'period': period, 'alpha':alpha, 'beta': beta ,'accuracy': accuracy}

	        df = df.append(register, ignore_index=True)


df.to_csv('./results/des_experiments_'+stock_name+'_'+str(year)+'.csv', header=True, index=False)